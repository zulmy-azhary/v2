...
const AboutContent: React.FC = () => {
  ...
  return (
    <div className="md:basis-3/5 flex flex-col gap-y-6 font-thin text-sm md:text-base text-gray tracking-tight relative">
      {contents.map((content, i) => (
        <Text key={i} className="leading-6">
          {content}
        </Text>
      ))}
      <Dots size={16} className="grid-cols-4 gap-3 md:gap-4 absolute rotate-12 -z-10 top-12 md:-top-12 lg:-top-6 xl:-top-24 right-0 md:right-20 lg:-right-24 xl:right-0" />
